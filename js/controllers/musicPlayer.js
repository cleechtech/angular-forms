...
.controller('MusicPlayerCtrl', function($scope, top100SongsEver){
	$scope.audioPlaylist = top100SongsEver.map(function(song, index, array){
		var parseTitle = song.displayName.split(' - ');
		return  { src: song.url, type: 'audio/ogg', artist: parseTitle[0], title: parseTitle[1] };
	})
})